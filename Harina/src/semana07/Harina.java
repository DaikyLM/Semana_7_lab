...
public class Harina extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		if(marca=="Blanca Flor"){pre=5.85;}
		else if(marca=="Favorita") {pre=3.85;}
		else if(marca=="Molitalia") {pre=3.00;}
		else {pre=3.20;}
		
		impC=cant*pre;
		
		if(cant>=12) {impD=impC*0.11;}
		else if(cant>=6 && cant<10) {impD=impC*0.09;}
		else if(cant>=3 && cant<15) {impD=impC*0.07;}
		else {impD=impC*0.05;}
		
		impP=impC-impD;
		
		txtS.setText("PROGRAMA HARINA \n\n");
