...
public class Golosina extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		...
		if(marca == "Cream Cracker"){ pre=9.0; }
		else if(marca =="Chomp"){ pre=7.4; }
		else if(marca =="Pícaras"){ pre=8.6; }
		else { pre=10.6; }
		impC = pre*cant;
		...
		if(cant>=12){ caramelo = 4*cant;}
		else if(cant>=8 && cant<12){ caramelo = 2*cant;}
		else if(cant>=4 && cant<8){ caramelo = 6;}
		else { caramelo=3; }
		
		txtS.setText("PROGRAMA GOLOSINA\n\n");
